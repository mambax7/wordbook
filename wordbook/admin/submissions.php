	...
	default:
		...
		if ( $numrows > 0 ) // That is, if there ARE submitted entries in the system
			...
			while ( list( $entryID, $categoryID, $term, $uid, $created) = $xoopsDB -> fetchrow( $resultS2 ) )
				...
				$modify = "<a href='entry.php?op=mod&entryID=" . $entryID . "'><img src=" . XOOPS_URL . "/modules/" . $xoopsModule->dirname() . "/images/icon/edit.gif ALT='"._AM_WB_EDITSUBM."'></a>";
				$delete = "<a href='entry.php?op=del&entryID=" . $entryID . "'><img src=" . XOOPS_URL . "/modules/" . $xoopsModule->dirname() . "/images/icon/delete.gif ALT='"._AM_WB_DELETESUBM."'></a>";
